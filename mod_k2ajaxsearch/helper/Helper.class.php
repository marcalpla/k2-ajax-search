 ...
 * @copyright	Copyright (C) 2012 Taleia Software
...
if(!defined('K2AJAXSearchHelper')) {
  define("K2AJAXSearchHelper", null);

class K2AJAXSearchHelper{
  function getAllCategories(&$categorylist, $name_prefix = ' - ', $rootlevel_prefix = 1){
    K2AJAXSearchHelper::getCategoryAndChildrens(0, 0, $categorylist, $name_prefix, $rootlevel_prefix); 
  }
  ...
  function getCategoryAndChildrens($id = 0, $level = 0, &$categorylist, $name_prefix = ' - ', $rootlevel_prefix = 1){
    $id = (int) $id;
    $db = &JFactory::getDBO();
	$user = &JFactory::getUser();
    ...
    if(!empty($results)){
      $level++;
      $prefix = '';
      for($i = $rootlevel_prefix; $i < $level; $i++) {
        $prefix .= $name_prefix;
      }
      ...
      foreach($results as $key=>$item) {
        ...
        $categorylist[] = $item; 
        K2AJAXSearchHelper::getCategoryAndChildrens($item->id, $level, $categorylist, $name_prefix, $rootlevel_prefix);           
      }
    }
  }

  function getAllExtrafieldsMultiSelect(&$extrafieldlist){
    K2AJAXSearchHelper::getExtrafieldsMultiSelect(0, $extrafieldlist);
  }

  function getExtrafieldsMultiSelect($id = 0, &$extrafieldlist){
    $db = &JFactory::getDBO();

    $query = "SELECT id, name, value FROM #__k2_extra_fields WHERE published = 1 AND type = 'multipleSelect' ";
